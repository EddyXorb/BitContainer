...
public:
	...
	template<class ...Args>
	constexpr BitContainer(Args... args) { insert(args...); }
...
inline constexpr auto BitContainer<BitContainerValueT>::getPos(value_type val)  -> std::size_t
{
	...
	throw std::runtime_error("Given BitContainerValue does not contain any set bit. Check derived class.");
	return 0;
}

template<class BitContainerValueT>
template<class ...Args>
inline constexpr auto BitContainer<BitContainerValueT>::insert(value_type first, Args ...args)->std::enable_if_t<are_same<value_type, Args...>(), void>
{
	insert(first);
	insert(args...);
}
